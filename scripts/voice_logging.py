...
Centralized logging utilities for the voice task management system.
...
- voice-errors.log: Dedicated error tracking (optional)
...
class VoiceLogger:
    """Centralized logging system for voice processing automation"""
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        if project_root is None:
            # Auto-detect project root (assuming this file is in scripts/ subdirectory)
    ...
    def _write_to_file(self, filepath: Path, content: str):
        ...
        try:
            ...
            print(f"[LOGGING ERROR] Failed to write to {filepath}: {e}")
    ...
    def update_run_stats(self, files_found: int = None, files_processed: int = None):
    ...
    def log_run_summary(self, files_found: int = None, files_processed: int = None, 
    ...
    def rotate_logs(self, max_size_mb: float = 10.0, keep_backups: int = 5):
        """
    ...
    def _rotate_single_log(self, log_file: Path, keep_backups: int):
...
def get_logger(project_root: Optional[Path] = None) -> VoiceLogger:
...
def log_run_summary(files_found: int, files_processed: int, errors: int, duration_seconds: float):
    """Legacy compatibility function - creates temporary logger"""
    logger = get_logger()
